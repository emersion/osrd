...
const getOccupancyBounds = (
  routeAspects: ProjectPathTrainResult['signal_updates'],
  timePosition: Date
): [number, number] => {
...
type TrainDetailsV2Props = {
  spaceTimeData: ProjectPathTrainResult;
};

const TrainDetailsV2 = ({ spaceTimeData }: TrainDetailsV2Props) => {
  const { consolidatedSimulation, selectedTrainId } = useSelector(
    (state: RootState) => state.osrdsimulation
  );

  // TODO TS2 : adapt useChartSynchro to TS2
  ...
  const occupancyBounds = useMemo(
    () => getOccupancyBounds(spaceTimeData.signal_updates, new Date(headPosition?.time)),
