...
import drawCurve, { drawCurveV2 } from 'modules/simulationResult/components/ChartHelpers/drawCurve';
...
import type { GevPreparedDataV2 } from './types';
import { createProfileSegmentV2 } from './utils';
...
export const createSpeedSpaceChart = (
  CHART_ID: string,
  resetChart: boolean,
  trainSimulation: GevPreparedDataV2,
  initialHeight: number,
  ref: React.RefObject<HTMLDivElement>,
  chart?: SpeedSpaceChart
) => {
  ...
  if (chart === undefined || resetChart) {
    const maxX = d3.max(
      trainSimulation.baseSpeedData,
      (speedObject) => speedObject.position
    ) as number;
    scaleX = defineLinear(maxX + 100);

    const maxY = d3.max(
      trainSimulation.baseSpeedData,
      (speedObject) => speedObject.speed
    ) as number;
    scaleY = defineLinear(maxY);
