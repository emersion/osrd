...
import type { LightRollingStock, PathProperties } from 'common/api/osrdEditoastApi';
...
interface SpeedSpaceSettingsV2Props {
  electrificationRanges: NonNullable<PathProperties['electrifications']>['values'];
...
const SpeedSpaceSettingsV2 = ({
  electrificationRanges,
  showSettings,
  speedSpaceSettings,
  onSetSettings,
  trainRollingStock,
}: SpeedSpaceSettingsV2Props) => {
  ...
  const runsOnlyThermal = (
    electricRanges: NonNullable<PathProperties['electrifications']>['values']
  ) =>
    !electricRanges.some(
      (range) => range.type === 'electrification'
      //    &&
...
export default SpeedSpaceSettingsV2;
