...
import SpeedSpaceSettingsV2 from './SpeedSpaceSettingsV2';
import type { SimulationPowerRestrictionRangeV2 } from './types';
...
export type SpeedSpaceChartV2Props = {
  ...
  pathProperties: NonNullable<PathProperties>;
  pathLength: number;
...
const SpeedSpaceChartV2 = ({
  ...
  const formattedTrainSimulation = useMemo(
    () =>
      prepareSpeedSpaceDataV2(
        trainSimulation,
        selectedTrainPowerRestrictions,
        pathProperties,
        pathLength
      ),
    [trainSimulation, pathProperties, pathLength]
  );
  ...
  const timeScaleRange: [Date, Date] = useMemo(() => {
    if (chart && formattedTrainSimulation) {
      ...
      return spaceScaleRange.map((position) =>
        interpolateOnPositionV2(formattedTrainSimulation, position)
      ) as [Date, Date];
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <SpeedSpaceSettingsV2
            electrificationRanges={pathProperties.electrifications!.values}
