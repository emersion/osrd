...
import getAxisValues, {
  getAxisValuesV2,
} from 'modules/simulationResult/components/ChartHelpers/drawHelpers';
import type { Chart } from 'reducers/osrdsimulation/types';

import { getElementWidth, buildStripe } from './ChartHelpers';
import type { ElectricalConditionSegmentV2 } from '../SpeedSpaceChart/types';
...
export const drawElectricalProfileV2 = (
  chart: Chart,
  classes: string,
  electricalConditionSegment: ElectricalConditionSegmentV2,
  groupID: string,
  keyValues: DrawingKeys,
  isStripe: boolean,
  isIncompatible: boolean,
  ...
  const electrificationType = electricalConditionSegment.electrification.type;
  ...
  const electrificationMode =
    electrificationType === 'electrification'
      ? electricalConditionSegment.electrification.voltage
      : undefined;

  // Get the different axis values based on rotation
  // x
  const xValues = getAxisValuesV2(electricalConditionSegment, keyValues, 0);
  // y
  const yValues = getAxisValuesV2(electricalConditionSegment, keyValues, 1);
  ...
  if (
    electrificationType === 'electrification' ||
    //   electricalConditionSegment.electrification.mode_handled) ||
    (electrificationType === 'neutral_section' &&
      electricalConditionSegment.electrification.lower_pantograph)
  )
    drawZone.call(addTextZone);
  ...
  drawZone
    ...
    .on('mouseover', (event) => {
      ...
      // add profile pop-up text
      if (electrificationType !== 'electrification') {
        drawZone
          .append('text')
          .attr('class', `data`)
          .attr('dominant-baseline', 'middle')
